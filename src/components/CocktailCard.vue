        ...
        <div v-if="ingredients.length > 0" class="cocktail-ingredients">
          ...
          <div class="ingredients-grid">
            <div v-for="(ingredient, index) in ingredients" :key="index" class="ingredient-item">
              ...
              <span v-if="ingredient.measure" class="ingredient-measure">{{ ingredient.measure }}</span>
            </div>
          </div>
        </div>
...
const ingredients = computed(() => {
  ...
  for (let i = 1; i <= 15; i++) {
    const ingredient = props.cocktail[`strIngredient${i}` as keyof Cocktail] as string
    const measure = props.cocktail[`strMeasure${i}` as keyof Cocktail] as string
...
// Обработка ошибок изображений теперь в компоненте ImageLoader
</script>
